              ...
              <div v-if="settingsCollapseFlag">
                ...
                <div class="card-body"
                  ...
                  <div
                      ...
                      role="alert"
                      v-if="$store.getters['deleteDonor/getDeleteMessage']!==null && $store.getters['deleteDonor/getIsDeleteError']"
                  >
                    {{ $store.getters['deleteDonor/getDeleteMessage'] }}
                  </div>
                </div>

                <div class="card-body" v-if="$store.getters.getDesignation===3 && designation===1">
                  <v-btn rounded color="primary" :disabled="$store.getters.getChangeAdminLoaderFlag || !enableEditing" :loading="$store.getters.getChangeAdminLoaderFlag" @click="changeHallAdminClicked()">
                    <v-icon
                        left
                        dark
                    >
                  ...
                  <div class="alert alert-danger mt-2" role="alert" v-if="$store.getters.getChangeAdminError!==null">
                    {{ $store.getters.getChangeAdminError }}
                  </div>
                  <div class="alert alert-success mt-2" role="alert" v-if="$store.getters.getChangeAdminSuccess!==null">
  ...
  methods: {
    async changeHallAdminClicked() {
      if(await this.$store.dispatch('changeHallAdmin',{donorId:this.$route.query.id})){
        setTimeout(()=>{this.designation=2},2500)
      }
    },
    async deleteDonor() {
    ...
    async promote() {
      ...
      if(await this.$store.dispatch('promote/promote', {
        ...
        setTimeout(()=>{this.designation=1},2500);
      }
    ...
    async demote() {
      ...
      if(await this.$store.dispatch('promote/promote', {
        ...
        setTimeout(()=>{this.designation = 0},2500);
      }
