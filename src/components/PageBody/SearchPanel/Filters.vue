...
<!--    A button to show the filters-->
    <button v-else="" class="h4 p-2 btn btn-outline-dark animated fadeInDown" style="width: 100%" @click="showFilter()">
      Show filters
    </button>
...
<!--        Input field for hall-->
        ...
        <select v-model="hall" class="form-control">
          <option v-for="(oneHall,index) in halls" v-if="$store.getters.getDesignation===3 || $store.getters.getHall=== halls.indexOf(oneHall) || halls.indexOf(oneHall)===7">
            {{oneHall}}
          </option>
        </select>
        <br>
...
<!--        A check box to search by availability-->
        <div class="custom-control custom-checkbox">
          <input v-model="availability" type="checkbox" class="custom-control-input" checked id="customCheck1">
...
<!--        The button for executing search-->
        <button type="button" class="btn btn-success" :disabled="$store.getters.getLoadingFlag" @click="searchClick()">
          Search
        </button>
        <br><br>
        ...
        methods: {
            searchClick(){
                ...
                let sendData={
                  ...
                  name: inputName,
                  bloodGroup: bloodGroups.indexOf(self.bloodGroup),
                  batch: inputBatch.toString(),
                  hall: halls.indexOf(self.hall),
                  availability: self.availability
                };
                console.log('Sending to api: ',sendData);
              this.axios.post('/search',sendData ).then(function (response) {
